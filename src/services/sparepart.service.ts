...
class SparepartService implements ISparepartService {
  ...
  public async addSparepart(data: SparepartsDTO): Promise<SparepartsDTO> {
    if (!data.partsName || typeof data.partsName !== "string" || data.partsName.trim() === "") {
      throw new AppError("Parts name is required and must be a valid string", 400);
    }
  ...
  public async getSparepartById(id: string): Promise<SparepartDTO | null> {
    if (!id || typeof id !== "string" || id.trim() === "") {
      throw new AppError("Sparepart ID is required and must be a valid string", 400);
    }
  ...
  public async getSparepartByName(nameQuery: string): Promise<SparepartDTO[]> {
    if (!nameQuery || typeof nameQuery !== "string") {
      throw new AppError("Name query is required and must be a valid string", 400);
    }
  ...
  public async updateSparepart(
    ...
    const { id: _, createdOn, createdBy, deletedOn, deletedBy, ...updateData } = data;
